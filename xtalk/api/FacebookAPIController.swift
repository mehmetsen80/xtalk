...
class FacebookAPIController{
    ...
    func fetchMyPhotos(){
        ...
        let graphRequest : FBSDKGraphRequest = FBSDKGraphRequest(graphPath: "me", parameters: ["fields":"photos.limit(18),picture"])
    ...
    func importMyFacebookPhoto(photo: FBMyPhoto, row: Int){
        ...
        let postString = "userid=\(pkUserID)&photo=\(photo.urlbase64!)&latitude=\(self.appDelegate.currentLocation.coordinate.latitude)&longitude=\(self.appDelegate.currentLocation.coordinate.longitude)&processType=IMPORTFACEBOOKPHOTO"
        //print(postString)
        request.HTTPBody = postString.dataUsingEncoding(NSUTF8StringEncoding)
        ...
        let task = NSURLSession.sharedSession().dataTaskWithRequest(request){
            ...
            do{
                let json = try NSJSONSerialization.JSONObjectWithData(data!, options: NSJSONReadingOptions.MutableContainers ) as! NSDictionary
                //print(json)
                self.delegate.didReceiveFacebookImportPhotoAPIResults(json, row: row)
